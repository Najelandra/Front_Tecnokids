...
export default class Proy extends React.Component {
    ...
    componentDidMount(prevProps) {
        ...
        setTimeout(() => {
            ...
            const urlUserData = `${process.env.HOST}${process.env.get_all_proyects}&userid=${this.state.idUser}`;
            ...
            const res = fetch(urlUserData, optionsUserData).then(function (response) {
                ...
                .then(function (data) {
                    ...
                    self.setState({ proyect_list: data.imagenes_proyecto });
    ...
    render() {
        ...
        if (this.state.proyect_list != '') {
            listaProyectos = <ListaProyectos proyects_list={this.state.proyect_list} idUser={this.state.idUser}></ListaProyectos>;
        }
        return (
            <div className={styles.container}>
                <Points active="5" username={this.state.username}></Points>
...
function ShowProyecto(params) {
    const [ver,
        setVer] = useState(true);

    

    function redirectToDetailProy(user_id, id)
    {
        
        
        sessionStorage.setItem('userid_proyect', user_id);
        sessionStorage.setItem('id_proyect', id);

        router.push('./detalleproy')
    }

    return (
        <div
            style={{
                'backgroundColor': '#cbe7ea',
                'paddingBottom': '10px',
                'borderRadius': '25px'
            }}>
            <div
                ...
                <a onClick={() => redirectToDetailProy( params.user_id , params.id)}>
                ...
                <div >
                    ...
                    <div className={styles.column1proyb}>
                        <MeGusta id={params.id} name={params.user_id} likes={params.likes} like_status={params.like_status} idUser={params.idUser}></MeGusta>
                    </div>
...
function ListaProyectos(props) {
    let proyectList = props.proyects_list;
    ...
    const items = []
    {
        proyectList.map((proyect, i) => {
            ...
            items.push(
                <div className={styles.column1proy} key={i}>
                    <ShowProyecto
                        id={proyect.id}
                        name={proyect.name}
                        type={proyect.type}
                        ...
                        like_status={proyect.like_user}
                        idUser={props.idUser}
                        user_name={proyect.name_user}
                        lesson={proyect.lesson}
                    >
                    </ShowProyecto>
                </div>
            )
...
function MeGusta(params) {
    ...
    function sendLike(){
        ...
        setTimeout(() => {
            ...
            const urlUserData = `${process.env.HOST}${process.env.set_like_by_user}${params.id}&userid=${params.idUser}`;
